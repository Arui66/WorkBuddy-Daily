...
> 🛠️ **本次更新**：重写了登录流程的**授权码捕获**逻辑——提交表单后不自动跟随重定向，改为**手动跟随整条重定向链**（含 30x 跳转与页面内 JS 跳转 `window.location`），从最终 URL 中解析 `authorization_code`，解决了部分账号登录后拿不到授权码导致失败的问题；同时补全了表单字段（自动照抄登录页所有 input 并回填真实手机号），并新增「验证码错误 / 需 OTP 二次验证」等失败原因提示。
...
[3/5] 提交登录...
  ✅ 授权码获取成功!
[4/5] 交换 Token...
  ✅ 登录成功! 身份: 13800000000 | AT过期: 2026-12-11
[5/5] 完成!
