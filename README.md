...
🔐 Token 永续 · ✅ 18 项成长任务 · 🏫 开学季活动 · 📱 小程序任务 · 🖥️ 桌面换血 · 🎮 8 项玩法 · 💰 三类查询 · 🎁 自动领奖 · 📊 全中文报告 · 📢 内置推送 · 🐧 青龙友好 · ☁️ GitHub Actions
...
一个脚本搞定 **WorkBuddy 成长中心 + 开学季活动 + 小程序任务** 的全部自动化：**Token 自动续期 → 积分/用量/成长查询 → 18 项成长任务 → 8 项互动玩法 → 开学季任务 + 大转盘抽奖 → 小程序成长任务 → 自动领奖 → 中文报告推送**，全流程无人值守，重复运行只补缺口、不重复领取。
...
> 总计 **33 项自动化任务**（31 项全自动 + 2 项需人工），其中仅 1 项无法自动完成（公益捐款）。
